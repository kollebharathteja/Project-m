...
if(isset($_GET['delete'])) {
    ...
    $delete_query = mysqli_query($con, "DELETE FROM faculty WHERE id = '$delete_id'");
    
    if($delete_query) {
        $message = "Faculty deleted successfully!";
        $message_type = "success";
    } else {
        $message = "Error deleting faculty: " . mysqli_error($con);
        $message_type = "danger";
    }
}
                            ...
                            <?php if(mysqli_num_rows($faculty_query) > 0): ?>
                                <?php while($faculty = mysqli_fetch_assoc($faculty_query)): ?>
                                    <tr>
                                        ...
                                        <td><?php echo htmlspecialchars($faculty['email'] ?? 'N/A'); ?></td>
                                        <td><?php echo $faculty['joining_date'] ? date('M d, Y', strtotime($faculty['joining_date'])) : 'N/A'; ?></td>
                                        <td>
                                            <a href="edit-faculty.php?id=<?php echo $faculty['id']; ?>" class="btn btn-sm btn-info">
                                                <i class="fas fa-edit"></i> Edit
                                            </a>
                                            <a href="?delete=<?php echo $faculty['id']; ?>" class="btn btn-sm btn-danger delete-btn">
